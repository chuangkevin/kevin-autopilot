...
test('web server exposes health and idea intake', async () => {
  ...
  try {
    ...
    assert.equal(pageBody.includes('現在先看這裡'), true)
    assert.equal(pageBody.includes('本輪建議候選'), true)
    assert.equal(pageBody.includes('如果判斷不對'), true)
    assert.equal(pageBody.includes('細節與證據'), true)
