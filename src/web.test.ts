...
test('web server exposes health and idea intake', async () => {
  ...
  try {
    ...
    // Idea tab now renders a compact swipeable card deck instead of a long list.
    ...
    assert.equal(pageAfterIdeaBody.includes('data-idea-track'), true)
    assert.equal(pageAfterIdeaBody.includes('data-idea-card'), true)
    assert.equal(pageAfterIdeaBody.includes('setIdeaIndex'), true)
...
test('idea tab renders textarea and transmit button', async () => {
  ...
  assert.ok(html.includes('data-idea-track'), 'missing carousel track marker')
