...
test('web server exposes health and idea intake', async () => {
  ...
  try {
    ...
    assert.equal(pageBody.includes('目前還不會「一直自動思考」'), true)
    assert.equal(pageBody.includes('首頁或 <code>/api/report</code> 被請求時即時跑一次觀察'), true)
    assert.equal(pageBody.includes('沒有「每幾分鐘一次」或「下一次執行時間」'), true)
