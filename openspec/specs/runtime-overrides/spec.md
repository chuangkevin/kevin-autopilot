...
## Purpose
Autopilot-owned whitelisted runtime config override layer: a `data/runtime-overrides.json` file written through a trusted-settings API and settings-page UI, merged non-destructively over the file config at the start of each decision so changes take effect without a container restart.
...
Kevin Autopilot SHALL read the effective config (file config merged with current runtime overrides) at the start of every observation cycle and at the start of every request handler whose behaviour depends on an overridable field.
...
- **THEN** Kevin Autopilot SHALL return JSON containing `overrides` (the current effective overrides) and `schema` (the whitelist with type and range hints).
...
- **THEN** Kevin Autopilot SHALL merge the body into the existing overrides, validate every present field, persist the result, and respond 200 with the new overrides.
