...
| **ai-core 小尾巴（非急件）** | OpenCode session 設計上**會累積對話歷史**，所以每 call 開關 session 是**正確設計**，不是浪費。radar 不應該 session reuse（會 O(n²) token 爆炸）。真正的 perf 大頭已經修在 radar 並行（commit `28...`，5-wide Promise.all batches）。剩下的小優化都在 ai-core 那邊：(a) HTTP keep-alive Agent 重用 TCP/TLS（每 call 省 ~20-100ms），(b) `cache_control` plumbing（要先確認 OpenCode 上游是否支援透傳）。沒量化前都不急。 |
